...
import type { bodyGetMessagesFilterPaging, ChatMessageResponse, EventGroup } from '@/types/ChatMessage.types'
import type {
...
const GroupChat = {
  createGroupChat: async (body: bodyCreateEventCode) => {
    const data = await http.post<APIResponse<{ message: string }>>('/group-chat/create', body)
    return data?.data
  },
  ...
  getGroupChatByUserId: async (userId: string) => {
    const data = await http.get<APIResponse<EventGroup[]>>(`/group-chat/${userId}`)
    return data?.data
  },
